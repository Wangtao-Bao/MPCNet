...
# dataset: NUAA-SIRST IRSTD-1K NUDT-SIRST IRSTD-Air
...
parser.add_argument("--begin_test", default=400, type=int)
...
parser.add_argument("--save", default=r'./log1', type=str, help="Save path of checkpoints")
parser.add_argument("--log_dir", type=str, default="./otherlogs/MPCNet", help='path of log files')
...
def train():
    ...
    if opt.resume:
        ckpt = torch.load('log/NUAA-SIRST/MSPNet_400_best.pth.tar')
        net.load_state_dict(ckpt['state_dict'])
        epoch_state = ckpt['epoch']
        total_loss_list = ckpt['total_loss']

    if opt.optimizer_name == 'Adam':
        opt.optimizer_settings = {'lr': 0.002}
        opt.scheduler_name = 'CosineAnnealingLR'
        opt.scheduler_settings = {'epochs': opt.epochs, 'eta_min': 1e-4, 'last_epoch': -1}
    ...
    if opt.optimizer_name == 'AdamW':
        opt.optimizer_settings = {'lr': 0.002, 'betas': (0.9, 0.999), "eps": 1e-8, "weight_decay": 1e-2,
                                  "amsgrad": False}
        opt.scheduler_name = 'CosineAnnealingLR'
...
class Net(nn.Module):
    def __init__(self, model_name, mode):
        ...
        if model_name == 'MPCNet':
            if mode == 'train':
                self.model = MPCNet(Train=True,size=opt.patchSize)
            else:
                self.model = MPCNet(Train=False,size=opt.patchSize)
